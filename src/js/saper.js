...
const checkVictory = () => {
  ...
  tiles.forEach((tile, i) => {
    ...
    if (!tile.classList.contains("tile--checked") && !bombs.includes(coord)) {
      gameWin = false;
    }
  });

  if (gameWin) {
    endScreenText(gameWin);
  }
};
...
const clickTile = (tile, number) => {
  ...
  if (bombs.includes(coord)) {
    endGame();
  } else {
    if (number > 0) {
      tile.innerHTML = number;
      tile.style.color = Object.keys(COLORS)[number - 1];
    };
...
const showAroundTile = (bool, coord) => {
  tiles.forEach((tile) => {
    ...
    for (let dx = -1; dx <= 1; dx++) {
      for (let dy = -1; dy <= 1; dy++) {
        if (`${parseInt(coord[0]) + dx},${parseInt(coord[1]) + dy}` === item) {
          bool
...
const setBomb = () => {
  ...
  tiles.forEach((item) => {
    ...
    random ? grids.push(`${x},${y}`) : bombs.push(`${x},${y}`);
